...
export const Pharma = () => {
	...
	useEffect(() => {
		const getPharmas = async () => {
			await axios
				.get(`${process.env.REACT_APP_BASE_URL}/pharma/get`)
	...
	const handleDelete = (resourceid) => {
		swal({
			...
			if (willDelete) {
				axios
					.delete(
						`${process.env.REACT_APP_BASE_URL}/pharma/delete/${resourceid}`
					)
					.then(() => {
						swal("Pharma has been deleted!", {
							icon: "success",
						}).then(() => {
				...
				swal("Pharma is not yet deleted!");
			}
		});
	};
			...
			Cell: ({
				...
				return (
					<div className="d-flex">
						<Link to={`/edit_pharma/${resourceid}`}>
							<span className="btn">Edit</span>
						</Link>
						<Link to={`/view_pharma/${resourceid}`}>
		...
		<div>
			...
			<div className="admin-wrapper col-12">
				<div className="admin-content">
					<div className="admin-head">Pharma</div>
